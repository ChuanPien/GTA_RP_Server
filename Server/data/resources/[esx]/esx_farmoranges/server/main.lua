...
local function HarvestKoda(source)
	...
	SetTimeout(Config.TimeToFarm, function()
		if PlayersHarvestingKoda[source] then
			...
			local orage = xPlayer.getInventoryItem('orage')

			if xPlayer.canCarryItem(orage, 1) then
...
local function TransformKoda(source)
	...
	SetTimeout(Config.TimeToProcess, function()
		if PlayersTransformingKoda[source] then
			...
			local pooch = xPlayer.getInventoryItem('oragejuice')
			...
			elseif kodaQuantity < 5 then
				...
				xPlayer.addInventoryItem('oragejuice', 1)
...
AddEventHandler('esx_farmoranges:startTransformKoda', function()
	...
	if not PlayersTransformingKoda[_source] then
		...
		TriggerClientEvent('esx:showNotification', _source, _U('transform_oragejuice'))
...
local function SellKoda(source)
	...
	SetTimeout(Config.TimeToSell, function()
		if PlayersSellingKoda[source] then
			local xPlayer = ESX.GetPlayerFromId(source)
			local poochQuantity = xPlayer.getInventoryItem('oragejuice').count

			if poochQuantity == 0 then
				TriggerClientEvent('esx:showNotification', source, _U('you_do_not_have_oragejuice'))
			else
				xPlayer.removeInventoryItem('oragejuice', 1)
...
AddEventHandler('esx_farmoranges:GetUserInventory', function(currentZone)
	...
	TriggerClientEvent('esx_farmoranges:ReturnInventory',
		...
		xPlayer.getInventoryItem('oragejuice').count,
		xPlayer.job.name,
		currentZone
	)
end)
