...
function TeleportPlayer(pos)
	...
	if #Values["goal"] > 1 then
		...
		function(data, menu)
			...
			if action == "戶外" or action == "訪客室" then

				if PlayerData.job.name ~= "police" then
					ESX.ShowNotification("你並沒有鑰匙!")
					return
				end
			end
...
Citizen.CreateThread(function()
	local blip = AddBlipForCoord(Config.Teleports["戶外"]["x"], Config.Teleports["戶外"]["y"], Config.Teleports["戶外"]["z"])
