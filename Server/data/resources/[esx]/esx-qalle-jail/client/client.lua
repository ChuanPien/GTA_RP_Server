...
function JailLogin()
	...
	ESX.ShowNotification("你在坐牢的途中睡著了!請呼叫上帝")
...
function UnJail()
	...
	ESX.Game.Teleport(PlayerPedId(), Config.Teleports["戶外"])

	ESX.TriggerServerCallback('esx_skin:getPlayerSkin', function(skin)
		TriggerEvent('skinchanger:loadSkin', skin)
	end)

	ESX.ShowNotification("恭喜你!出獄了!")
end
...
function InJail()
	...
	Citizen.CreateThread(function()
		...
		while jailTime > 0 do
			...
			ESX.ShowNotification("你還剩下" .. jailTime .. "年監獄時光!")
		...
		while true do
			...
			for p, v in pairs(Config.Teleports) do
				...
				if DistanceCheck <= 7.5 then
					...
					ESX.Game.Utils.DrawText3D(v, "[~g~E~s~]開門", 1)
...
function PackPackage(packageId)
	...
	while Packaging do
		...
		if PackPercent >= 100 then
			...
			ESX.Game.Utils.DrawText3D(Package, "包裝" .. math.ceil(tonumber(PackPercent)) .. "%", 0.4)
		end
...
function DeliverPackage(packageId)
	...
	while Packaging do
		...
		if not IsEntityAttachedToEntity(packageId, PlayerPedId()) then
			...
			ESX.Game.Utils.DrawText3D(DeliverPosition, "[~g~E~s~]給予", 0.4)
...
function OpenJailMenu()
	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'jail_prison_menu',
		{
			...
			align    = 'top-left',
			elements = {
				{ label = "入獄", value = "jail_closest_player" },
		...
		elseif action == "unjail_player" then
			...
			ESX.TriggerServerCallback("esx-qalle-jail:retrieveJailedPlayers", function(playerArray)
				...
				if #playerArray == 0 then
					ESX.ShowNotification("你並沒有坐牢!")
					return
				end

				for i = 1, #playerArray, 1 do
					table.insert(elements, {label = "囚犯: " .. playerArray[i].name .. " |監牢時光: " .. playerArray[i].jailTime .. "年", value = playerArray[i].identifier })
				end
