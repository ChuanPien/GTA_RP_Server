...
AddEventHandler('esx_kr_shops:RemoveItemFromShop', function(number, count, item)
  ...
  local identifier =  ESX.GetPlayerFromId(src).identifier
        ...
        elseif data[1].count ~= count then
            ...
            if xPlayer.canCarryItem(data[1].item, data[1].count) then
                    ...
                    TriggerEvent('esx_joblogs:AddInLog', "shop", "shopd", xPlayer.name,count,ESX.GetItemLabel(item),data[1].count - count,number)
                    TriggerClientEvent('mythic_notify:client:SendAlert', xPlayer.source, { type = 'inform', text = count .. ESX.GetItemLabel(item)})
                end)
            end
        ...
        elseif data[1].count == count then
            ...
            if xPlayer.canCarryItem(data[1].item, data[1].count) then
                ...
                xPlayer.addInventoryItem(data[1].item, count)
                TriggerEvent('esx_joblogs:AddInLog', "shop", "shopd", xPlayer.name,count,ESX.GetItemLabel(item),"0",number)
                TriggerClientEvent('mythic_notify:client:SendAlert', xPlayer.source, { type = 'inform', text = count .. ESX.GetItemLabel(item)})
            end
        end
    end)
    ...
    function(items)
      ...
      MySQL.Async.fetchAll(
        ...
        if data[1] == nil then
            ...
            TriggerEvent('esx_joblogs:AddInLog', "shop", "shopo", xPlayer.name,ItemCount,ESX.GetItemLabel(Item),ItemCount,id)
            TriggerClientEvent('mythic_notify:client:SendAlert', xPlayer.source, { type = 'inform', text = ItemCount .. ESX.GetItemLabel(Item)})
        ...
        elseif data[1].price == Price then
                ...
                xPlayer.removeInventoryItem(Item, ItemCount)
                TriggerEvent('esx_joblogs:AddInLog', "shop", "shopo", xPlayer.name,ItemCount,ESX.GetItemLabel(item),data[1].count + ItemCount,id)
                TriggerClientEvent('mythic_notify:client:SendAlert', xPlayer.source, { type = 'inform', text = ItemCount .. ESX.GetItemLabel(item)})


        elseif data ~= nil and data[1].price ~= Price then
                Wait(250)
                TriggerClientEvent('mythic_notify:client:SendAlert', xPlayer.source, { type = 'error', text = '此物品已經上架'})
            end
        end)  
    end)
end)
  ...
  local xPlayer = ESX.GetPlayerFromId(src)
        ...
        'SELECT * FROM owned_shops WHERE ShopNumber = @Number',
        {
            ...
            if xPlayer.getMoney() < ItemCount * result[1].price then
                TriggerClientEvent('mythic_notify:client:SendAlert', src, { type = 'error', text = '現金不足'})
                --TriggerClientEvent('esx:showNotification', src, '~r~現金不足')
            elseif ItemCount <= 0 then
                TriggerClientEvent('mythic_notify:client:SendAlert', src, { type = 'error', text = '數量錯誤'})
                --TriggerClientEvent('esx:showNotification', src, '~r~數量錯誤')
            elseif xPlayer.canCarryItem(result[1].item, ItemCount) then
                xPlayer.removeMoney(ItemCount * result[1].price)
                xPlayer.addInventoryItem(result[1].item, ItemCount)
                TriggerClientEvent('mythic_notify:client:SendAlert', src, { type = 'success', text = ItemCount .. result[1].label})
                TriggerClientEvent('mythic_notify:client:SendAlert', src, { type = 'error', text = '$' .. ItemCount * result[1].price})
                ...
                if result[1].count ~= ItemCount then
                    ...
                    TriggerEvent('esx_joblogs:AddInLog', "shop", "shopb", xPlayer.name,ItemCount,result[1].label,ItemCount * result[1].price,result[1].count - ItemCount,id)
                elseif result[1].count == ItemCount then
                    ...
                    TriggerEvent('esx_joblogs:AddInLog', "shop", "shopb", xPlayer.name,ItemCount,result[1].label,ItemCount * result[1].price,"0",id)
                end
            else
...
local identifier = ESX.GetPlayerFromId(src).identifier
    ...
    MySQL.Async.fetchAll('SELECT * FROM owned_shops WHERE ShopNumber = @ShopNumber AND identifier = @identifier',
        {
            ['@ShopNumber'] = id,
            ['@identifier'] = identifier,
        }, function(result)
  ...
  local identifier = ESX.GetPlayerFromId(_source).identifier
    ...
    MySQL.Async.fetchAll('SELECT money FROM owned_shops WHERE ShopNumber = @ShopNumber AND identifier = @identifier',{['@ShopNumber'] = id,['@identifier'] = identifier,}, function(result)
        ...
        if result[1].money >= price * count then
            ...
            TriggerClientEvent('esx:showNotification', _source, '~r~你的商店資金不足')
        end
    end)
end)
...
local identifier = ESX.GetPlayerFromId(source).identifier
    ...
    if result[1].identifier == '0' then
        ...
        if xPlayer.getMoney() >= price then
            ...
            TriggerClientEvent('mythic_notify:client:SendAlert', _source, { type = 'success', text = '購買成功'})
            TriggerClientEvent('mythic_notify:client:SendAlert', _source, { type = 'error', text = '$' .. price})
            TriggerEvent('esx_joblogs:AddInLog', "shop", "shop", xPlayer.name,number)
        else    
            TriggerClientEvent('mythic_notify:client:SendAlert', _source, { type = 'error', text = '無法購買'})
        end
...
local identifier = ESX.GetPlayerFromId(source).identifier
    ...
    MySQL.Async.fetchAll(
        ...
        if os.time() - result[1].LastRobbery <= 900 then
            ...
            TriggerClientEvent('esx:showNotification', xPlayer.source, '~r~你的商店因搶劫而關門,請等待' .. math.floor((900 - time) / 60) .. '分鐘')
            return
        end
        ...
        if xPlayer.getMoney() >= amount then
            ...
            TriggerClientEvent('mythic_notify:client:SendAlert', _source, { type = 'inform', text = '存入$' .. amount})
        else
            TriggerClientEvent('mythic_notify:client:SendAlert', _source, { type = 'error', text = '現金不足'})
        end
    end)
end)
  ...
  MySQL.Async.fetchAll(
    ...
    if os.time() - result[1].LastRobbery <= 900 then
        ...
        if result[1].money >= amount then
            ...
            TriggerClientEvent('mythic_notify:client:SendAlert', _source, { type = 'inform', text = '領出$' .. amount})
        else
            TriggerClientEvent('mythic_notify:client:SendAlert', _source, { type = 'error', text = '資金不足'})
        end
    ...
    function(result)
      ...
      if result[1].money == 0 and result2[1] == nil then
        MySQL.Async.fetchAll("UPDATE owned_shops SET identifier = @identifiers, ShopName = @ShopName WHERE identifier = @identifier AND ShopNumber = @Number",
        {
            ...
            TriggerClientEvent('esx_kr_shops:removeBlip', -1)
            TriggerClientEvent('esx_kr_shops:setBlip', -1)
            TriggerClientEvent('mythic_notify:client:SendAlert', src, { type = 'inform', text = '出售'})
            else
                TriggerClientEvent('mythic_notify:client:SendAlert', src, { type = 'error', text = '需清空商店'})
            end
        end)
    end)
end)
