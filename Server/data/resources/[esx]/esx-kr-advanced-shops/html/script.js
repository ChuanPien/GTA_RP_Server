...
$(function() {
		window.addEventListener('message', function(event) {
			...
			for (i = 0; i < event.data.result.length; i++) {
				...
				$('#wrapper').append(
					`<div class = "image" id = ${event.data.result[i].item} label = ${event.data.result[i].label} count = ${event.data.result[i].count} price = ${event.data.result[i].price}>
						...
						<h5 class = "h4">庫存: ${event.data.result[i].count}</h5>
					</div>`
				);
			}
			...
			if (event.data.owner == true) {
				...
				$('#wrapper').append(
					`
					<button class = "button" id = "bossactions" style = "position: absolute; right: 15px; top: 5px;">老闆選單</button>
					`
				);
			}
				...
				var i;
					for (i = 0; i < cb.length; i++) { 
						...
						$('#cart').append(
							...
							<h6>$${cb[i].price}/個</h4>
							<h6>庫存: ${cb[i].count}</h4>
							<input type="text" id = ${cb[i].item} count = ${cb[i].count} class = "textareas" placeholder = "購買數量"></textarea>
							</div>`
								);
							};
							...
							$('#cart').append(
							`
							<button class = "button" id = "buybutton" style = "position: absolute; right: 15px; top: 5px;">購買</button>
							<button class = "button" id = "back" style = "position: absolute; left: 15px; top: 5px;">返回</button>
							`
						);
					});
			});
			...
			$("body").on("click", "#buybutton", function() {
				...
				for (i = 0; i < value.length; i++) {
					...
					if (parseInt(count) >= parseInt(value[i].value) && parseInt(value[i].value) != 0 && isNumber) {
						...
						$.post('http://esx-kr-advanced-shops/notify', JSON.stringify({msg : "~r~數量錯誤"}));
					}
				}
			});
