...
ESX.RegisterServerCallback("esx_inventoryhud:getPlayerInventory1", function(source, cb, target)
    local targetXPlayer = ESX.GetPlayerFromId(target)
...
RegisterServerEvent('esx_inventoryhud:disableTargetInv1')
AddEventHandler('esx_inventoryhud:disableTargetInv1', function(target)
    local _source = source
    local _target = target
    TriggerClientEvent("esx_inventoryhud:disableOpen", _target)
    -- TriggerClientEvent("esx_inventoryhud:disableOpenShop", _target)
    TriggerClientEvent("esx_invnetoryhud:setOpenedPlayerId", _source, _target)
    TriggerClientEvent('mythic_notify:client:SendAlert', _target, { type = 'error', text = '背包被開啟'})
end)
