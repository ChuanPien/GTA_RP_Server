			...
			if isActive == 0 then
				ESX.TriggerServerCallback('esx_carthief:anycops', function(anycops)
					if anycops >= Config.CopsRequired then
						...
						data = {dispatchCode = 'carthief', caller = _U('caller_local'), coords = Config.VehicleSpawnPoint.Pos, netId = NetworkGetNetworkIdFromEntity(playerPed), length = 10000}
						TriggerServerEvent('wf-alerts:svNotify',data)

						exports['mythic_notify']:DoHudText('error','走私開始')
						exports['mythic_notify']:DoHudText('error','位置已暴露')
						...
						ClearAreaOfVehicles(Config.VehicleSpawnPoint.Pos.x, Config.VehicleSpawnPoint.Pos.y, Config.VehicleSpawnPoint.Pos.z, 0.0, false, false, false, false, false)
  ...
  while true do
    Wait(1000)
		if isTaken == 1 and isDelivered == 0 and not (GetVehiclePedIsIn(GetPlayerPed(-1), false) == car) then
			exports['mythic_notify']:DoHudText('note', '回到車上')
			Wait(50000)
			if isTaken == 1 and isDelivered == 0 and not (GetVehiclePedIsIn(GetPlayerPed(-1), false) == car) then
				exports['mythic_notify']:DoHudText('error','回到車上')
				Wait(10000)
