...
local Bullet = { 3218215474, 1593441988, 584646201, -1716589765, 324215364, 736523883, -270015777, -1074790547, -2084633992, -1357824103, -1660422300, 2144741730, 487013001, 2017895192, -494615257, -1654528753, 100416529, 205991906, 1119849093 }
...
Citizen.CreateThread(function()
	...
	while true do
		...
		if not IsPedInAnyVehicle(GetPlayerPed(-1)) and ESX.PlayerData.job and ESX.PlayerData.job.name == 'ambulance' then
...
function Start(ped)
	...
	while checking do
		...
		if distance < 2.0 then
			DrawText3D(x,y,z, '[~g~E~s~]檢查傷口', 0.4)
...
function OpenDeathMenu(player)
	...
	table.insert(elements, {label = '驗傷', value = 'deathcause'})
	table.insert(elements, {label = '查看傷口位置', value = 'damage'})
	...
	function(data, menu)
		...
		exports['mythic_notify']:DoHudText('inform', re)
...
RegisterNetEvent('esx_ambulance:cheakbody')
AddEventHandler('esx_ambulance:cheakbody', function(ped)
	OpenDeathMenu(ped)
end)