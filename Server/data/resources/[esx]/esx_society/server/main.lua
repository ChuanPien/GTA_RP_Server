	...
	if xPlayer.job.name == society.name then
		TriggerEvent('esx_addonaccount:getSharedAccount', society.account, function(account)
			if amount > 0 and account.money >= amount then
				...
				TriggerClientEvent('mythic_notify:client:SendAlert', xPlayer.source, { type = 'success', text = '$' ..amount})
			else
				TriggerClientEvent('mythic_notify:client:SendAlert', xPlayer.source, { type = 'error', text = '公款不足'})
			end
		end)
	end
end)
...
ESX.RegisterServerCallback('esx_society:getEmployees', function(source, cb, society)
	if Config.EnableESXIdentity then
		...
		MySQL.Async.fetchAll('SELECT name, identifier, jobs, job_grade FROM users WHERE job = @job ORDER BY job_grade DESC', {
			['@job'] = society
		}, function (result)
			local employees = {}
...
ESX.RegisterServerCallback('esx_society:setJob', function(source, cb, identifier, job, grade, type)
	...
	if isBoss then
		...
		if xTarget then
			...
			if type == 'hire' then
				TriggerClientEvent('mythic_notify:client:SendAlert', xTarget.source, { type = 'success', text = '入取' .. job})
			elseif type == 'promote' then
				TriggerClientEvent('mythic_notify:client:SendAlert', xTarget.source, { type = 'success', text = '升階'})
			elseif type == 'fire' then
				TriggerClientEvent('mythic_notify:client:SendAlert', xTarget.source, { type = 'error', text = '解雇' .. job})
			end
...
TriggerEvent('cron:runAt', 23, 41, WashMoneyCRON)
