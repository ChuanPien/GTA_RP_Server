		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'ambulance_confirm', {
			...
			if data2.current.value == 'yes' then
				...
				if ESX.PlayerData.job and ESX.PlayerData.job.name == 'admin' then
						...
						if success then
							...
							if vehicleData.model == Config.AmbulanceHeli or vehicleData.model == Config.AmbulanceHeli2 then
								ESX.Game.SpawnVehicle(vehicleData.model, Config.Zones.ShopOutsideAmbulanceHeli.Pos, Config.Zones.ShopOutsideAmbulanceHeli.Heading, function(vehicle)
									...
									local vehicle = vehicleData.model
					...
					exports['mythic_notify']:DoHudText('error', '錯誤位階')
				end
			else
				menu2.close()
			end
						...
						if success then
							...
							if vehicleData.model == Config.PoliceHeli then
								ESX.Game.SpawnVehicle(vehicleData.model, Config.Zones.ShopOutsidePoliceHeli.Pos, Config.Zones.ShopOutsidePoliceHeli.Heading, function(vehicle)
									...
									FreezeEntityPosition(playerPed, false)
									SetEntityVisible(playerPed, true)
									TriggerServerEvent('esx_joblogs:AddInLog', 'vehicleshop', 'selfcarbuy', GetPlayerName(PlayerId()), "警車" .. vehicleName, generatedPlate, vehicleData.price)
								end)
							else
				...
				ESX.TriggerServerCallback('esx_advancedvehicleshop:buyVehicleA', function(success)
					if success then
						...
						ESX.Game.SpawnVehicle(vehicleData.model, Config.Zones.ShopOutsideAircraft.Pos, Config.Zones.ShopOutsideAircraft.Heading, function(vehicle)
							...
							FreezeEntityPosition(playerPed, false)
							SetEntityVisible(playerPed, true)
							TriggerServerEvent('esx_joblogs:AddInLog', 'vehicleshop', 'selfcarbuy', GetPlayerName(PlayerId()), "飛機" .. vehicleName, generatedPlate, vehicleData.price)
						end)
					else
						exports['mythic_notify']:DoHudText('error', '現金不足')
					end
				...
				ESX.TriggerServerCallback('esx_advancedvehicleshop:buyVehicleB', function(success)
					if success then
						...
						ESX.Game.SpawnVehicle(vehicleData.model, Config.Zones.ShopOutsideBoat.Pos, Config.Zones.ShopOutsideBoat.Heading, function(vehicle)
							...
							FreezeEntityPosition(playerPed, false)
							SetEntityVisible(playerPed, true)
							TriggerServerEvent('esx_joblogs:AddInLog', 'vehicleshop', 'selfcarbuy', GetPlayerName(PlayerId()), "船隻".. vehicleName, generatedPlate, vehicleData.price)
						end)
					else
						exports['mythic_notify']:DoHudText('error', '現金不足')
					end
				...
				ESX.TriggerServerCallback('esx_advancedvehicleshop:buyVehicleC', function(success)
					if success then
						...
						ESX.Game.SpawnVehicle(vehicleData.model, Config.Zones.ShopOutsideCar.Pos, Config.Zones.ShopOutsideCar.Heading, function(vehicle)
							...
							FreezeEntityPosition(playerPed, false)
							SetEntityVisible(playerPed, true)
							TriggerServerEvent('esx_joblogs:AddInLog', 'vehicleshop', 'selfcarbuy', GetPlayerName(PlayerId()), "車輛".. vehicleName, generatedPlate, vehicleData.price)
						end)
					else
						exports['mythic_notify']:DoHudText('error', '現金不足')
					end
				...
				ESX.TriggerServerCallback('esx_advancedvehicleshop:buyVehicleT', function(success)
					if success then
						...
						ESX.Game.SpawnVehicle(vehicleData.model, Config.Zones.ShopOutsideTruck.Pos, Config.Zones.ShopOutsideTruck.Heading, function(vehicle)
							...
							FreezeEntityPosition(playerPed, false)
							SetEntityVisible(playerPed, true)
							TriggerServerEvent('esx_joblogs:AddInLog', 'vehicleshop', 'selfcarbuy', GetPlayerName(PlayerId()), vehicleName, generatedPlate, vehicleData.price)
						end)
					else
						exports['mythic_notify']:DoHudText('error', '現金不足')
					end
				...
				if IsPedSittingInAnyVehicle(playerPed) then
					...
					if GetPedInVehicleSeat(vehicle, -1) == playerPed then
						...
						if vehicleData == nil then
							exports['mythic_notify']:DoHudText('error', '無法出售')
							Citizen.Wait(10000)
						else
							...
							if Config.LicenseEnable then
								ESX.TriggerServerCallback('esx_license:checkLicense', function(hasdriverLicense)
									if hasdriverLicense then
										...
										exports['mythic_notify']:DoHudText('error', '無證照')
									end
						...
						if ESX.PlayerData.job and ESX.PlayerData.job.name == 'admin' then
							ESX.TriggerServerCallback('esx_advancedvehicleshop:resellVehicleAJ', function(vehicleSold)
								if vehicleSold then
									...
									exports['mythic_notify']:DoHudText('inform', '出售' .. CurrentActionData.label .. CurrentActionData.price)
								else
									exports['mythic_notify']:DoHudText('error', '無法出售')
								end
					...
					if Config.UseAircraftShop then
						ESX.TriggerServerCallback('esx_advancedvehicleshop:resellVehicleA', function(vehicleSold)
							if vehicleSold then
								ESX.Game.DeleteVehicle(CurrentActionData.vehicle)
								exports['mythic_notify']:DoHudText('inform', '出售' .. CurrentActionData.label .. CurrentActionData.price)
								TriggerServerEvent('esx_joblogs:AddInLog', 'vehicleshop', 'carsold', GetPlayerName(PlayerId()),  "飛機" .. CurrentActionData.label, CurrentActionData.plate ,CurrentActionData.price)
							else
								exports['mythic_notify']:DoHudText('error', '無法出售')
							end
						end, CurrentActionData.plate, CurrentActionData.model)
					...
					if Config.UseBoatShop then
						ESX.TriggerServerCallback('esx_advancedvehicleshop:resellVehicleB', function(vehicleSold)
							if vehicleSold then
								ESX.Game.DeleteVehicle(CurrentActionData.vehicle)
								exports['mythic_notify']:DoHudText('inform', '出售' .. CurrentActionData.label .. CurrentActionData.price)
								TriggerServerEvent('esx_joblogs:AddInLog', 'vehicleshop', 'carsold', GetPlayerName(PlayerId()), "船隻" .. CurrentActionData.label, CurrentActionData.plate ,CurrentActionData.price)
							else
								exports['mythic_notify']:DoHudText('error', '無法出售')
							end
						end, CurrentActionData.plate, CurrentActionData.model)
					...
					if Config.UseCarShop then
						ESX.TriggerServerCallback('esx_advancedvehicleshop:resellVehicleC', function(vehicleSold)
							if vehicleSold then
								ESX.Game.DeleteVehicle(CurrentActionData.vehicle)
								exports['mythic_notify']:DoHudText('inform', '出售' .. CurrentActionData.label .. '$' .. CurrentActionData.price)
								TriggerServerEvent('esx_joblogs:AddInLog', 'vehicleshop', 'carsold', GetPlayerName(PlayerId()), "車" .. CurrentActionData.label, CurrentActionData.plate ,CurrentActionData.price)
							else
								exports['mythic_notify']:DoHudText('error', '無法出售')
							end
						end, CurrentActionData.plate, CurrentActionData.model)
					...
					if Config.UseTruckShop then
						ESX.TriggerServerCallback('esx_advancedvehicleshop:resellVehicleT', function(vehicleSold)
							if vehicleSold then
								ESX.Game.DeleteVehicle(CurrentActionData.vehicle)
								exports['mythic_notify']:DoHudText('inform', '出售' .. CurrentActionData.label .. CurrentActionData.price)
								TriggerServerEvent('esx_joblogs:AddInLog', 'vehicleshop', 'carsold', GetPlayerName(PlayerId()), CurrentActionData.label, CurrentActionData.plate ,CurrentActionData.price)
							else
								exports['mythic_notify']:DoHudText('error', '無法出售')
							end
						end, CurrentActionData.plate, CurrentActionData.model)
						...
						if Config.LicenseEnable then
							ESX.TriggerServerCallback('VIPSCRIPT:VIPSCRIPT', function(success)
								if success then
									...
									exports['mythic_notify']:DoHudText('error', '非VIP')
								end
							end)
						else
							BuyVIPMenu()
