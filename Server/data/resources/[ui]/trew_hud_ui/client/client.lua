...
Citizen.CreateThread(function()
    while true do
        ...
        if IsPedInAnyVehicle(PlayerPedId(), false) and not inVeh then
            ...
            if driver == PlayerPedId() and GetVehicleClass(veh) ~= 13 and GetVehicleClass(veh) ~= 14 and
                GetVehicleClass(veh) ~= 15 and GetVehicleClass(veh) ~= 16 and GetVehicleClass(veh) ~= 17 and
                GetVehicleClass(veh) ~= 21 then
                inVeh = true
                Citizen.Wait(50)
                vehPlate = GetVehicleNumberPlateText(veh)
                Citizen.Wait(1)
                ESX.TriggerServerCallback('trew_hud_ui:getMileage', function(hasKM)
                    ...
                    if IsVehicleOnAllWheels(veh) then
                        dist =
                            GetDistanceBetweenCoords(oldPos.x, oldPos.y, oldPos.z, curPos.x, curPos.y, curPos.z, true)
                    else
                        dist = 0
                    end
                    hasKM = hasKM + dist
                    TriggerServerEvent('trew_hud_ui:addMileage', vehPlate, hasKM)
                    inVeh = false
...
Citizen.CreateThread(function()
	...
	while true do
		...
		if IsPedInAnyVehicle(player, false) and vehicleIsOn then
			...
			local vehicleGear = round(showKM,1) .. 'KM'
			...
			if has_value(vehiclesCars, vehicleClass) == true and vehicleClass ~= 8 then
                ...
                if not seatbeltIsOn and PlayerData.job.name ~= 'admin' then
