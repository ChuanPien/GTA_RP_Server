...
function setUniform(uniform, playerPed)
	TriggerEvent('skinchanger:getSkin', function(skin)
		...
		if uniformObject then
			...
			exports['mythic_notify']:DoHudText('error', '無合適制服')
		end
	end)
end
			...
			if Config.EnableESXService then
				ESX.TriggerServerCallback('esx_service:isInService', function(isInService)
					if isInService then
						...
						exports['mythic_notify']:DoHudText('inform', '下班')
					end
				end, 'police')
			end
		end
				...
				if not isInService then
					...
					ESX.TriggerServerCallback('esx_service:enableService', function(canTakeService, maxInService, inServiceCount)
						if not canTakeService then
							exports['mythic_notify']:DoHudText('error', '警力已滿')
						else
							awaitService = true
							...
							TriggerServerEvent('esx_service:notifyAllInService', notification, 'police')
							TriggerEvent('esx_policejob:updateBlip')
							exports['mythic_notify']:DoHudText('inform', '上班')
						end
					end, 'police')
...
function OpenPoliceActionsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'police_actions', {
	  ...
	  if data.current.value == 'citizen_interaction' then
			...
			local elements = {
				{label = ('上銬'), value = 'handcuff'},
				{label = ('解銬'), value = 'uncuff'},
				...
				elseif DoesEntityExist(vehicle) then
					...
					elseif action == 'hijack_vehicle' then
						if IsAnyVehicleNearPoint(coords.x, coords.y, coords.z, 3.0) then
							...
							exports['mythic_notify']:DoHudText('inform', '解鎖')
						end
...
function OpenIdentityCardMenu(player)
	ESX.TriggerServerEvent('esx_policejob:getOtherPlayerData', function(data)
		local elements = {}
...
function OpenBodySearchMenu(player)
	TriggerEvent("esx_inventoryhud:openPlayerInventory1", GetPlayerServerId(player), GetPlayerName(player))
end

-- 上銬
RegisterNetEvent('esx_policejob:cuffcheck')
AddEventHandler('esx_policejob:cuffcheck', function()
	local target, distance = ESX.Game.GetClosestPlayer()
	playerheading = GetEntityHeading(GetPlayerPed(-1))
	playerlocation = GetEntityForwardVector(PlayerPedId())
	playerCoords = GetEntityCoords(GetPlayerPed(-1))
...
RegisterNetEvent('esx_policejob:ropecheck')
AddEventHandler('esx_policejob:ropecheck', function()
...
RegisterNetEvent('esx_policejob:unlockingcuffs')
AddEventHandler('esx_policejob:unlockingcuffs', function()
	local target, distance = ESX.Game.GetClosestPlayer()
	playerheading = GetEntityHeading(GetPlayerPed(-1))
	playerlocation = GetEntityForwardVector(PlayerPedId())
	playerCoords = GetEntityCoords(GetPlayerPed(-1))
...
RegisterNetEvent('esx_policejob:thief')
AddEventHandler('esx_policejob:thief', function()
	local closestPlayer, closestDistance = ESX.Game.GetClosestPlayer()
	OpenBodySearchMenu(closestPlayer)
end)
...
function IsAbleToSteal(targetSID, err)
	ESX.TriggerServerCallback('esx_thief:getValue', function(result)
		...
		if result.value then
			...
			exports['mythic_notify']:DoHudText('error', '對方未投降')
		end
	end, targetSID)
end
	...
	ESX.TriggerServerCallback('esx_policejob:getFineList', function(fines)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'fine_category', {
			...
			if Config.EnablePlayerManagement then
				TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(player), 'society_police_2', _U('fine_total', data.current.fineLabel), data.current.amount)
			else
			...
			exports['mythic_notify']:DoHudText('success', '' .. data.current.fineLabel .. '$' .. data.current.amount)
			TriggerServerEvent('esx_joblogs:AddInLog',"police" ,"SendFine" ,GetPlayerName(PlayerId()) ,GetPlayerName(player) ,data.current.fineLabel ,data.current.amount)
...
function LookupVehicle()
	ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'lookup_vehicle', {
		...
		if not data.value or length < 2 or length > 8 then
			exports['mythic_notify']:DoHudText('error', '錯誤車牌')
		else
			ESX.TriggerServerCallback('esx_policejob:getVehicleInfos', function(retrivedInfo)
				menu.close()
				if not retrivedInfo.owner then
					ESX.ShowAdvancedNotification('~b~警用系統', '[車輛資訊]', '~b~車牌:' .. retrivedInfo.plate .. '\n~r~車輛擁有者:未知', 'CHAR_CALL911')
				else
					ESX.ShowAdvancedNotification('~b~警用系統', '[車輛資訊]', '~b~車牌:' .. retrivedInfo.plate .. '\n~g~車輛擁有者:' .. retrivedInfo.owner, 'CHAR_CALL911')
				end
			end, data.value)
		end
	end, function(data, menu)
...
function OpenVehicleInfosMenu(vehicleData)
	ESX.TriggerServerCallback('esx_policejob:getVehicleInfos', function(retrivedInfo)
		if not retrivedInfo.owner then
			ESX.ShowAdvancedNotification('警用系統', '[車輛資訊]', '~b~車牌:' .. retrivedInfo.plate .. '\n~r~車輛擁有者:未知', 'CHAR_CALL911')
		else
			ESX.ShowAdvancedNotification('警用系統', '[車輛資訊]', '~b~車牌:' .. retrivedInfo.plate .. '\n~g~車輛擁有者:' .. retrivedInfo.owner, 'CHAR_CALL911')
		end
			...
			ESX.TriggerServerCallback('esx_policejob:buyWeapon', function(bought)
				if bought then
					if data.current.price > 0 then
						exports['mythic_notify']:DoHudText('inform', '購買' .. data.current.weaponLabel .. data.current.price)
					...
					exports['mythic_notify']:DoHudText('error', '現金不足')
				end
...
function OpenWeaponComponentShop(components, weaponName, parentShop)
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armory_buy_weapons_components', {
		...
		if data.current.hasComponent then
			exports['mythic_notify']:DoHudText('error', '已裝備')
		else
			ESX.TriggerServerCallback('esx_policejob:buyWeapon', function(bought)
				if bought then
					if data.current.price > 0 then
	...
	ESX.TriggerServerCallback('esx_policejob:getStockItems', function(items)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_get_item_count', {
				...
				if not count then
					exports['mythic_notify']:DoHudText('error', '錯誤數值')
				else
					menu2.close()
					menu.close()
	...
	while true do
			...
			if IsControlJustReleased(0, 38) and ESX.PlayerData.job and ESX.PlayerData.job.name == 'police' then
				...
				elseif CurrentAction == 'menu_armory' then
					...
					elseif playerInService then
						...
						exports['mythic_notify']:DoHudText('error', '下班')
					end
		...
		if IsControlJustReleased(0, 167) and not isDead and ESX.PlayerData.job and ESX.PlayerData.job.name == 'police' and not ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'police_actions') then
			...
			elseif playerInService then
				...
				exports['mythic_notify']:DoHudText('error', '須穿制服')
			end
		end

		if IsControlJustReleased(0, 38) and currentTask.busy then
			exports['mythic_notify']:DoHudText('error', '取消扣押')
			ESX.ClearTimeout(currentTask.task)
...
function StartHandcuffTimer()
	...
	handcuffTimer.task = ESX.SetTimeout(Config.HandcuffTimer, function()
		exports['mythic_notify']:DoHudText('error', '手銬鬆脫')
		TriggerEvent('esx_policejob:unrestrain')
