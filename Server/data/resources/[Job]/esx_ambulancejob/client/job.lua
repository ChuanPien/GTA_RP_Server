			...
			if data.current.value == 'billing' then
				...
				if amount == nil then
					...
					if closestPlayer == -1 or closestDistance > 5.0 then
						...
						TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(closestPlayer), 'society_ambulance', '醫護局', amount)
						TriggerServerEvent("esx:ambulancejob", GetPlayerName(closestPlayer), amount)
						exports['mythic_notify']:DoHudText('success', '帳單$' .. amount)
						TriggerServerEvent('esx_joblogs:AddInLog',"ambulance" ,"SendFine" ,GetPlayerName(PlayerId()) ,GetPlayerName(closestPlayer) ,"醫護單" ,amount)
					end
				end
			end, function(data, menu)
			menu.close()
				...
				ESX.TriggerServerCallback('esx_ambulancejob:getItemAmount', function(quantity)
					if quantity > 0 then
						...
						if health > 0 then
							...
							exports['mythic_notify']:DoHudText('error', '需急救')
						end
					else
						exports['mythic_notify']:DoHudText('error', '無繃帶')
					end
				end, 'bandage')
			...
			elseif data.current.value == 'big' then
				...
				ESX.TriggerServerCallback('esx_ambulancejob:getItemAmount', function(quantity)
					if quantity > 0 then
						...
						exports['mythic_notify']:DoHudText('error', '無醫療包')
					end
				end, 'medikit')

			elseif data.current.value == 'cheakbody' then
				TriggerEvent('esx_ambulance:cheakbody',GetPlayerName(closestPlayer))
	...
	ESX.TriggerServerCallback('esx_ambulancejob:getItemAmount', function(quantity)
		if quantity > 0 then
			...
			if IsPedDeadOrDying(closestPlayerPed, 1) then
				...
				exports['mythic_notify']:DoHudText('error', '無需急救')
			end
		else
			exports['mythic_notify']:DoHudText('error', '無醫療包')
		end
