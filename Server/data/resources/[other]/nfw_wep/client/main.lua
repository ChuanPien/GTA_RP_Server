...
AddEventHandler('nfw_wep:pAmmo', function()
    ...
    if table.includes(pistol, currentWeaponHash) then
        exports['mythic_notify']:DoHudText('inform', '12發彈藥')
        AddAmmoToPed(ped, currentWeaponHash, 12)
    else
        exports['mythic_notify']:DoHudText('error', '彈藥型號錯誤')
        TriggerServerEvent('returnItem', item)
    end
end)
...
AddEventHandler('nfw_wep:arAmmo', function()
    ...
    if(ammo >= 250 or ammo + 50 > 250) then
        exports['mythic_notify']:DoHudText('note', '彈藥已滿')
        TriggerServerEvent('returnItem', item)
        return
    end

    if table.includes(ar, currentWeaponHash) then
        exports['mythic_notify']:DoHudText('inform', '30發彈藥')
        AddAmmoToPed(ped, currentWeaponHash, 30)
    else
        exports['mythic_notify']:DoHudText('error', '彈藥型號錯誤')
        TriggerServerEvent('returnItem', item)
    end
