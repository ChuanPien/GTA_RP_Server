...
UTK = {
    ...
    alarmblip,
    text = { -- Texts
        loudstart = "[~r~E~w~]~y~安裝炸藥",
        silentstart = "[~r~E~w~]~y~開鎖",
        usecard = "[~r~E~w~]~y~使用ID Card",
        usethermal = "[~r~E~w~]~y~使用炸藥",
        usehack = "[~r~E~w~]~y~駭入保全系統",
        uselockpick = "[~r~E~w~]~y~使用開鎖工具",
        usesearch = "[~r~E~w~]~y~搜尋",
        lootcash= "[~r~E~w~]~y~拿取現金",
        lootgold = "[~r~E~w~]~y~拿取金條",
        lootdia = "[~r~E~w~]~y~拿取鑽石",
        card = "使用ID Card",
        thermal = "安裝炸藥",
                                ...
                                if dst <= 4 then
                                    ...
                                    if dst <= 1 and IsControlJustReleased(0, 38) then
                                        ...
                                        if output then
                                            ...
                                            self:IdCard()
                                        elseif not output then
                                            exports['mythic_notify']:DoHudText("error", "無ID Card")
                                        end
                                        end, "id_card")
                                    end
                                end
                                    ...
                                    if dst <= 1 and IsControlJustReleased(0, 38) then
                                        ESX.TriggerServerCallback("utk_oh:checkItem", function(output)
                                            if output then
                                                ...
                                                self:Plant()
                                            elseif not output then
                                                exports['mythic_notify']:DoHudText("error", "無鋁熱炸藥")
                                            end
                                        end, "thermal_charge")
                                    end
                                end
                                        ...
                                        if dst <= 1 and IsControlJustReleased(0, 38) then
                                            ESX.TriggerServerCallback("utk_oh:checkItem", function(output)
                                                if output then
                                                    ...
                                                    self:Lockpick()
                                                elseif not output then
                                                    exports['mythic_notify']:DoHudText("error", "無迴紋針")
                                                end
                                            end, "lockpick")
                                        end
                                    end
...
function UTK:Lockpick()
    ...
    Citizen.Wait(6500)
    exports['mythic_notify']:DoHudText("success", "階段1"..self.text.stage)
    SetEntityCoords(ped, loc.x, loc.y, loc.z, 1, 0, 0, 1)
    SetEntityHeading(ped, loc.h)
    TaskPlayAnim(ped, animDict, "a_uncuff", 8.0, 8.0, 6000, 1, 1, 0, 0, 0)
    Citizen.Wait(6500)
    exports['mythic_notify']:DoHudText("success", "階段2 "..self.text.stage)
...
function UTK:Hack()
    ...
    Citizen.Wait(2000)
    Brute()
    exports['mythic_notify']:DoHudText("inform", "[My Computer] [外接裝置] [HackConnect.exe]")
...
function DrawText3D(x, y, z, text, scale) local onScreen, _x, _y = World3dToScreen2d(x, y, z) local pX, pY, pZ = table.unpack(GetGameplayCamCoords()) SetTextScale(scale, scale) SetTextFont(0) SetTextProportional(1) SetTextEntry("STRING") SetTextCentre(true) SetTextColour(255, 255, 255, 215) AddTextComponentString(text) DrawText(_x, _y) local factor = (string.len(text)) / 700 DrawRect(_x, _y + 0.0150, 0.095 + factor, 0.03, 41, 11, 41, d0) end
        ...
        if HasScaleformMovieLoaded(scaleform) and UsingComputer then
            ...
            if GetScaleformMovieFunctionReturnBool(ClickReturn) then
                ...
                if program == 82 and not Hacking then
                    ...
                    PopScaleformMovieFunctionVoid()
                    Hacking = true
                    exports['mythic_notify']:DoHudText("inform", "尋找IP位置")
                elseif program == 83 and not Hacking and Ipfinished then
                    ...
                    Hacking = true
                    exports['mythic_notify']:DoHudText("inform", "尋找密碼")
                ...
                elseif Hacking and program == 84 then
                    ...
                    Hacking = false
                    Ipfinished = true
                    exports['mythic_notify']:DoHudText("inform", "[BruteForce.exe]")
...
AddEventHandler("utk_oh:vaulttimer", function(method)
    if method == 1 then
        exports['mythic_notify']:DoHudText("error", "150秒後釋放毒氣")
        UTK.starttimer = true
        UTK.vaulttime = 150
        Citizen.CreateThread(function()
            while true do
        ...
        ESX.TriggerServerCallback("utk_oh:gettotalcash", function(result)
            ...
            exports['mythic_notify']:DoHudText("success", text)
        end)
    else
        exports['mythic_notify']:DoHudText("error", "150秒後釋放毒氣")
        UTK.starttimer = true
    ...
    Citizen.CreateThread(function()
        ...
        while true do
            ...
            if UTK.begingas then
                ...
                local dst1 = GetDistanceBetweenCoords(playerloc, 252.71, 218.22, 99.5, true)
                local dst2 = GetDistanceBetweenCoords(playerloc, 262.78, 213.22, 99.5, true)
    ...
    Citizen.CreateThread(function()
        while true do
            ...
            if UTK.begingas then
                ...
                UTK.stage2break = true
                UTK.stage4break = true
                -- exports['mythic_notify']:DoHudText("error", "金庫大門關閉")
...
--     local player = ESX.GetPlayerData()
    -- if player.job.name == "police" then
        -- if toggle == 1 then
            -- TriggerServerEvent('esx_outlawalert:bankInProgress')
            ...
            if dst <= 50 then
                ...
                if PlayerData.job.name == "admin" then
                    if dst <= 1.5 then
                        ...
                        DrawText3D(x, y, z, doortext, 0.50)
                        if IsControlJustReleased(0, 38)  then
