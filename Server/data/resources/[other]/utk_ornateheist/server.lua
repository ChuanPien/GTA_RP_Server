...
local mincash = 5500 -- minimum amount of cash a pile holds
local maxcash = 6000 -- maximum amount of cash a pile can hold
            ...
            if copcount >= mincops then
                if method == 1 then
                    ...
                    if item >= 1 then
                        TriggerClientEvent('chat:addMessage', -1, {
                            ...
                            args = { "🚔 ^7警政署", '目前中央銀行搶劫進行中，市民請勿靠近。若經警方勸導不離者，將帶回警局偵辦' }
                        })
                        yPlayer.removeInventoryItem("thermal_charge", 1)
                        data = {dispatchCode = 'bank', caller = ('bank'), coords = {x = 231.0, y = 214.88, z = 105.96}, netId = NetworkGetNetworkIdFromEntity(playerPed), length = 10000}
                        TriggerEvent('wf-alerts:svNotify',data)
                        TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'error', text = '鋁熱炸藥'})
                        cb(true)
                        info.stage = 1
                        info.style = 1
                        info.locked = true
                ...
                elseif method == 2 then
                    ...
                    if item >= 1 then
                        ...
                        data = {dispatchCode = 'bank', caller = ('bank'), coords = {x = 231.0, y = 214.88, z = 105.96} , netId = NetworkGetNetworkIdFromEntity(playerPed), length = 10000}
                        TriggerEvent('wf-alerts:svNotify',data)
                        TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'error', text = '迴紋針'})
                        info.stage = 1
                        info.style = 2
                        info.locked = true
                        cb(true)
...
ESX.RegisterServerCallback("utk_oh:gettotalcash", function(source, cb)
    cb(totalcash)
    TriggerEvent('esx_joblogs:AddInLog', "crime", "bank", xPlayer.name,totalcash,totalgold,totaldia)
end)
...
AddEventHandler("utk_oh:removeitem", function(itemname)
    ...
    TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'error', text = itemname}) 
end)
