	...
	if (item.showmenu){
		...
		for (const contract of contracts) {
			...
			if(contract.cargo_type == 1) {
				list_item += '<div data-tooltip="炸藥"><img class="rounded-circle" src="img/explosive-1.png" width="30"></div>';
			}else if(contract.cargo_type == 2) {
				list_item += '<div data-tooltip="易燃氣體"><img class="rounded-circle" src="img/flamable-2.png" width="30"></div>';
			}else if(contract.cargo_type == 3) {
				list_item += '<div data-tooltip="易燃液體"><img class="rounded-circle" src="img/flamable-3.png" width="30"></div>';
			}else if(contract.cargo_type == 4) {
				list_item += '<div data-tooltip="易燃固體"><img class="rounded-circle" src="img/flamable-4.png" width="30"></div>';
			}else if(contract.cargo_type == 5) {
				list_item += '<div data-tooltip="有毒物質"><img class="rounded-circle" src="img/toxic-6.png" width="30"></div>';
			}else if(contract.cargo_type == 6) {
				list_item += '<div data-tooltip="腐蝕性物質"><img class="rounded-circle" src="img/corrosive-8.png" width="30"></div>';
			}
			if(contract.fragile == 1) {
				list_item += '<div data-tooltip="易碎貨物"><img class="rounded-circle" src="img/fragile.png" width="30"></div>';
			}
			if(contract.valuable == 1) {
				list_item += '<div data-tooltip="高價貨物"><img class="rounded-circle" src="img/valuable.png" width="30"></div>';
			}
			if(contract.fast == 1) {
				list_item += '<div data-tooltip="限時貨物"><img class="rounded-circle" src="img/fast.png" width="30"></div>';
			}
			list_item += `
							</div>
						</div>
						<button onclick="startJob(` + contract.contract_id + `,` + contract.reward + `,` + contract.distance + `)" class="btn btn-blue btn-darken-2 white">開始送貨</button>
					</div>
				</li>
			</ul>
			`;
		...
		$('#skills-desc').empty();
		$('#skills-desc').append("擁有技能點: " + users.skill_points);
			...
			list_item += `
				<div class="card"> <img src="` + truck.img + `" class="card-img-top" width="100%">
					<div class="card-body pt-0 px-0">
						<div class="d-flex flex-row justify-content-between mb-0 mt-3 px-3"> <span class="text-muted">車名</span>
							<h6>` + truck.name + `</h6>
						</div>
						<hr class="mt-2 mx-3">
						<div class="d-flex flex-row justify-content-between px-3 pb-4">
							<div class="d-flex flex-column"><span class="text-muted">價錢</span></div>
						...
						<div class="d-flex flex-row justify-content-between p-3 mid">
							<div class="d-flex flex-column"><small class="text-muted mb-1">引擎</small>
						...
						<div class="mx-3 mt-3 mb-2"><button onclick="buyTruck('` + key + `','` + truck.price + `')" type="button" class="btn btn-blue btn-darken-2 white btn-block"><small>購買</small></button></div>
					</div>
				</div>
				`;
		}
		...
		for (const truck of myTrucks) {
			...
			if(truck.driver == 0){
				...
				$('#diagnostic-body').append(`
					<div class="media d-flex">
						<div class="media-body text-left">
							<h3 class="` + color + `">` + truck.body + ` %</h3><span>維護車身 (` + new Intl.NumberFormat(config.formatacao.location, { style: 'currency', currency: config.formatacao.moeda }).format((100-truck.body)*config.valor_reparo.body) + `)</span>
						</div>
				...
				$('#diagnostic-engine').append(`
					<div class="media d-flex">
						<div class="media-body text-left">
							<h3 class="` + color + `">` + truck.engine + ` %</h3><span>維護引擎 (` + new Intl.NumberFormat(config.formatacao.location, { style: 'currency', currency: config.formatacao.moeda }).format((100-truck.engine)*config.valor_reparo.engine) + `)</span>
						</div>
						<div class="align-self-center">
							<i class="fas fa-tools ` + color + ` font-large-2 float-right"></i>
						</div>
				...
				$('#diagnostic-transmission').append(`
					<div class="media d-flex">
						<div class="media-body text-left">
							<h3 class="` + color + `">` + truck.transmission + ` %</h3><span>維護變速器箱 (` + new Intl.NumberFormat(config.formatacao.location, { style: 'currency', currency: config.formatacao.moeda }).format((100-truck.transmission)*config.valor_reparo.transmission) + `)</span>
						</div>
						<div class="align-self-center">
							<i class="fas fa-tools ` + color + ` font-large-2 float-right"></i>
						</div>
				...
				$('#diagnostic-wheels').append(`
					<div class="media d-flex">
						<div class="media-body text-left">
							<h3 class="` + color + `">` + truck.wheels + ` %</h3><span>維護輪胎 (` + new Intl.NumberFormat(config.formatacao.location, { style: 'currency', currency: config.formatacao.moeda }).format((100-truck.wheels)*config.valor_reparo.wheels) + `)</span>
						</div>
						<div class="align-self-center">
							<i class="fas fa-tools ` + color + ` font-large-2 float-right"></i>
						</div>
				...
				<li class="d-flex justify-content-between">
					<div class="d-flex flex-row align-items-center"><img src="` + config.concessionaria[truck.truck_name].img + `" class="img-radius img-width" alt="User-Profile-Image">
						<div class="ml-2">
							...
							<div class="d-flex flex-row mt-1 text-black-50 date-time">
								<div>
									<i class="fas fa-tools"></i><span class="ml-2">車身: ` + truck.body + `%</span>
								</div>
								<div class="ml-3">
									<i class="fas fa-tools"></i><span class="ml-2">引擎: ` + truck.engine + `%</span>
								</div>
								<div class="ml-3">
									<i class="fas fa-tools"></i><span class="ml-2">變速箱: ` + truck.transmission + `%</span>
								</div>
								<div class="ml-3">
									<i class="fas fa-tools"></i><span class="ml-2">輪胎: ` + truck.wheels + `%</span>
								</div>
							</div>
						</div>
					</div>
					<div class="d-flex flex-row align-items-center">
						...
						<button onclick="sellTruck('` + truck.truck_id + `','` + truck.truck_name + `')" class="btn btn-red btn-accent-4 white">出售</button>
					</div>
				</li>
				`;
		}
		...
		list_item = `
			<div class="col-12 mt-3 mb-1">
				<h4 class="text-uppercase">招募員工</h4>
			</div>
			`;
		for (const driver of drivers) {
			if (driver.user_id == null || driver.user_id == undefined){
				list_item += `
					<div class="card user-card">
						<div class="card-block">
							...
							<p class="text-muted">薪資: ` + new Intl.NumberFormat(config.formatacao.location, { style: 'currency', currency: config.formatacao.moeda }).format(driver.price) + ` + ` + new Intl.NumberFormat(config.formatacao.location, { style: 'currency', currency: config.formatacao.moeda }).format(driver.price_per_km) + `/km</p>
							<hr>
							<p class="text-muted">危險貨物</p>
							<ul class="list-unstyled activity-leval">
								` + getDriverLevelHTML(driver.product_type) + `
							</ul>
							<p class="text-muted">運輸距離</p>
							<ul class="list-unstyled activity-leval">
								` + getDriverLevelHTML(driver.distance) + `
							</ul>
							<p class="text-muted">高價貨物</p>
							<ul class="list-unstyled activity-leval">
								` + getDriverLevelHTML(driver.valuable) + `
							</ul>
							<p class="text-muted">易碎貨物</p>
							<ul class="list-unstyled activity-leval">
								` + getDriverLevelHTML(driver.fragile) + `
							</ul>
							<p class="text-muted">限時貨物</p>
							<ul class="list-unstyled activity-leval">
					...
					<li class="d-flex justify-content-between">
						<div class="d-flex flex-row align-items-center">
							...
							<div class="ml-2">
								...
								<div class="d-flex flex-row mt-1 text-black-50 date-time">
									<div>
										<i class="fas fa-coins"></i><span class="ml-2">薪資: ` + new Intl.NumberFormat(config.formatacao.location, { style: 'currency', currency: config.formatacao.moeda }).format(driver.price) + ` + ` + new Intl.NumberFormat(config.formatacao.location, { style: 'currency', currency: config.formatacao.moeda }).format(driver.price_per_km) + `/km</span>
									</div>
									<div class="ml-3">
										<i class="fas fa-medal"></i><span class="ml-2">員工技能: 危險貨物 (` + driver.product_type + `) 運輸距離 (` + driver.distance + `) 高價貨物 (` + driver.valuable + `) 易碎貨物 (` + driver.fragile + `) 限時貨物 (` + driver.fast + `)</span>
									</div>
								</div>
							</div>
						</div>
				...
				<li class="d-flex justify-content-between">
					<div class="d-flex flex-row align-items-center"><i class="fa fa-check-circle checkicon"></i>
						<div class="ml-2">
							...
							<div class="d-flex flex-row mt-1 text-black-50 date-time">
								<div><i class="fas fa-coins"></i></i><span class="ml-2">金額: ` + new Intl.NumberFormat(config.formatacao.location, { style: 'currency', currency: config.formatacao.moeda }).format(loan.remaining_amount) + `</span></div>
								<div class="ml-3"><i class="fas fa-coins"></i></i><span class="ml-2">利息/日: ` + new Intl.NumberFormat(config.formatacao.location, { style: 'currency', currency: config.formatacao.moeda }).format(loan.day_cost) + `</span></div>
							</div>
						</div>
					</div>
					<div class="d-flex flex-row align-items-center">
						<button onclick="payLoan('` + loan.id + `')" class="btn btn-red btn-accent-4 white">還款</button>
					</div>
				</li>
			`;
...
function getDriverAvailableTrucksHTML(myTrucks,driver,config){
	...
	if (has_truck == null) {
		html = '<option selected="selected">無</option>' + html;
	}else{
		html = '<option driver_id="' + driver.driver_id + '">無</option>' + html;
	}
	return html;
}

function getMyTruckHTML(truck){
	return truck.driver==0 ? `<button onclick="spawnTruck(` + truck.truck_id + `)" class="btn btn-blue btn-darken-2 white white mr-2">駕駛</button> <button onclick="setDriver(null,'` + truck.truck_id + `')" class="btn btn-blue btn-darken-2 white white mr-2">取消選擇</button>` : `<button onclick="setDriver('0','` + truck.truck_id + `')" class="btn btn-blue btn-darken-2 white white mr-2">選擇貨車</button>`
}
