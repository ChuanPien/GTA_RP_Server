...
function gsrcheck(source, identifier)
	...
	if Config.UseCharName then
		...
		if gsrData[identifier] ~= nil then
			TriggerClientEvent('mythic_notify:client:SendAlert', Source, { type = 'success', text = '明顯硝煙反應', length = 5000 })
    	else
			TriggerClientEvent('mythic_notify:client:SendAlert', Source, { type = 'error', text = '無硝煙反應', length = 5000 })
    	end
	else
    	if gsrData[identifier] ~= nil then
			TriggerClientEvent('mythic_notify:client:SendAlert', Source, { type = 'success', text = '明顯硝煙反應', length = 5000 })
...
AddEventHandler('GSR:Status2', function(playerid)
    ...
    if Config.UseCharName then
    	...
    	if gsrData[identifier] ~= nil then
			TriggerClientEvent('mythic_notify:client:SendAlert', Source, { type = 'success', text = '明顯硝煙反應', length = 5000 })
			TriggerClientEvent('GSR:inspect', source)
    	else
			TriggerClientEvent('mythic_notify:client:SendAlert', Source, { type = 'error', text = '無硝煙反應', length = 5000 })
			TriggerClientEvent('GSR:inspect', source)
    	end
