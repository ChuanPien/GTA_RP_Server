                ...
                if ( radarInfo.fwdXmit ) then  
                    ...
                    if ( DoesEntityExist( fwdVeh ) and IsEntityAVehicle( fwdVeh ) ) then
                        ...
                        if ( fwdVehSpeed > radarInfo.fastLimit and not radarInfo.fwdFastLocked ) then
                            ...
                            ESX.ShowAdvancedNotification('~b~警用測速系統', '[前車超速]', '~b~車牌:' .. fwdPlate .. '\n~r~車速:' .. fwdVehSpeed .. 'Km/h', 'CHAR_CALL911')
                        end
                ...
                if ( radarInfo.bwdXmit ) then 
                    ...
                    if ( DoesEntityExist( bwdVeh ) and IsEntityAVehicle( bwdVeh ) ) then
                        ...
                        if ( bwdVehSpeed > radarInfo.fastLimit and not radarInfo.bwdFastLocked ) then 
                            ...
                            ESX.ShowAdvancedNotification('~b~警用測速系統', '[後車超速]', '~b~車牌:' .. bwdPlate .. '\n~r~車速:' .. bwdVehSpeed .. 'Km/h', 'CHAR_CALL911', false, true)
                        end 
...
Citizen.CreateThread( function()
    while true do 
        ...
        -- LShift E
        if ( IsDisabledControlPressed( 0, 155 ) and IsDisabledControlJustPressed( 1, 153 ) and IsPedSittingInAnyVehicle( ped ) ) then 
            TriggerEvent( 'wk:radarRC' )
        end 

        -- E
        if IsDisabledControlJustPressed( 0, 153 ) then
            ResetFrontFast()
            ResetRearFast()
        end
