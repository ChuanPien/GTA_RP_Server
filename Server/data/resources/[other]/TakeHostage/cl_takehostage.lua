...
function takeHostage()
	...
	for i=1, #hostageAllowedWeapons do
		if GetSelectedPedWeapon(PlayerPedId()) == GetHashKey(hostageAllowedWeapons[i]) then
			canTakeHostage = true 
	...
	if not canTakeHostage then 
		exports['mythic_notify']:DoHudText('error', '須持武器')
	end
...
Citizen.CreateThread(function()
	while true do 
		if holdingHostage then
			...
			DrawText3D(playerCoords.x,playerCoords.y,playerCoords.z,"[~g~G~s~]釋放[~r~H~s~]殺害")
