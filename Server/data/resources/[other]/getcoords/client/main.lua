...
RegisterCommand("sw", function(source)
...
AddEventHandler("GetGoords:ControlSwitch", function()
	...
	Citizen.CreateThread(function()
		while ControlSwitch do
			Citizen.Wait(0)
			if IsControlJustPressed(0, Keys["N+"]) and ControlSwitchTime and ControlSwitch then -- "~"
				PlaySoundFrontend(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", false)
				TriggerServerEvent('GetGoords:CheckDisplayPermission')
...
function SetDisplay(bool, TypeCoords)
    ...
    local PedCoordsC = "{x = "..round(x,2)..", y = "..round(y,2)..", z = "..round(z,2).."}"
