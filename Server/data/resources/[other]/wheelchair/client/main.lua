	...
	while true do
		...
		if DoesEntityExist(closestObject) then
			...
			if GetDistanceBetweenCoords(pedCoords, sitCoords, true) <= 2.0 then
				DrawText3Ds(sitCoords, "[~g~E~s~]坐", 0.4)
			...
			if GetDistanceBetweenCoords(pedCoords, pickupCoords, true) <= 2.0 then
				DrawText3Ds(pickupCoords, "[~y~G~s~]推", 0.4)
...
PickUp = function(wheelchairObject)
	...
	if closestPlayer ~= nil and closestPlayerDist <= 1.5 then
		if IsEntityPlayingAnim(GetPlayerPed(closestPlayer), 'anim@heists@box_carry@', 'idle', 3) then
			exports['mythic_notify']:DoHudText('note', '已有人')
			return
		end
	end
...
DrawText3Ds = function(coords, text, scale)
	...
	DrawRect(_x, _y + 0.0150, 0.030 + factor, 0, 41, 11, 41, 100)
end
