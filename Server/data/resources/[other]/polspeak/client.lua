...
mainMenu = NativeUI.CreateMenu("警用廣播系統","")
...
function AddMenuAnotherMenu(menu)
    local submenu = _menuPool:AddSubMenu(menu, "停下(車) »", "")
    ...
    if item == shout1 then
      ...
      TriggerServerEvent("InteractSound_SV:PlayWithinDistance", 50.0, "stop_vehicle", 0.6)
    end
    if item == shout2 then
      TriggerServerEvent("InteractSound_SV:PlayWithinDistance", 50.0, "stop_vehicle-2", 0.6)
    end
    if item == shout3 then
      TriggerServerEvent("InteractSound_SV:PlayWithinDistance", 50.0, "stop_the_f_car", 0.6)
    end
    if item == shout4 then
      TriggerServerEvent("InteractSound_SV:PlayWithinDistance", 50.0, "stop_or_executed", 0.6)
    end
    if item == shout5 then
      TriggerServerEvent("InteractSound_SV:PlayWithinDistance", 50.0, "stop_or_i_kill", 0.6)
    end
    end	
end

function Stop(menu)
    local submenu4 = _menuPool:AddSubMenu(menu, "停下(人) »", "")
    ...
    if item == shout1 then
      TriggerServerEvent("InteractSound_SV:PlayWithinDistance", 50.0, "dont_make_me", 0.6)
    end	
    if item == shout2 then
      TriggerServerEvent("InteractSound_SV:PlayWithinDistance", 50.0, "stop_dont_move", 0.6)
    end
    if item == shout3 then
      TriggerServerEvent("InteractSound_SV:PlayWithinDistance", 50.0, "give_yourself_up", 0.6)
    end	
    if item == shout4 then
      TriggerServerEvent("InteractSound_SV:PlayWithinDistance", 50.0, "stay_right_there", 0.6)
    end	
    if item == shout5 then
      TriggerServerEvent("InteractSound_SV:PlayWithinDistance", 50.0, "freeze_lspd", 0.6)
    end			
    end	
end
...
function Clear(menu)
    local submenu2 = _menuPool:AddSubMenu(menu, "清空區域 »", "")
    ...
    if item == shout1 then
      TriggerServerEvent("InteractSound_SV:PlayWithinDistance", 50.0, "clear_the_area", 0.6)
    end		
    if item == shout2 then
      TriggerServerEvent("InteractSound_SV:PlayWithinDistance", 50.0, "this_is_the_lspd", 0.6)
    end	
    if item == shout3 then
      TriggerServerEvent("InteractSound_SV:PlayWithinDistance", 50.0, "move_along_people", 0.6)
    end	
    if item == shout4 then
      TriggerServerEvent("InteractSound_SV:PlayWithinDistance", 50.0, "get_out_of_here_now", 0.6)
    end	
    if item == shout5 then
      TriggerServerEvent("InteractSound_SV:PlayWithinDistance", 50.0, "disperse_now", 0.6)
    end	
    end	
end
...
function Insults(menu)
    local submenu3 = _menuPool:AddSubMenu(menu, "警告 »", "")
    ...
    if item == shout1 then
      TriggerServerEvent("InteractSound_SV:PlayWithinDistance", 50.0, "its_over_for_you", 0.6)
    end	
    if item == shout2 then
      TriggerServerEvent("InteractSound_SV:PlayWithinDistance", 50.0, "you_are_finished_dhead", 0.6)
    end	
    if item == shout3 then
      TriggerServerEvent("InteractSound_SV:PlayWithinDistance", 50.0, "cant_hide_boi", 0.6)
    end	
    if item == shout4 then
      TriggerServerEvent("InteractSound_SV:PlayWithinDistance", 50.0, "drop_a_missile", 0.6)
    end
    if item == shout5 then
    ...
    if PlayerData.job ~= nil and PlayerData.job.name == 'police' then
      _menuPool:ProcessMenus()
        if IsControlJustPressed(1, 244) then
          ...
          exports['mythic_notify']:DoHudText('error', '限警車')
          end
        end
      end
    end
